...
!(async () => {
  ...
  console.log('默认前6个ck开团，变量CITYNUM');
  // if (exchangeFlag) {
  //   console.log(`脚本自动抽奖`)
  // } else {
...
function dotask(taskId, taskToken, actionType = 0) {
  return new Promise((resolve) => {
    $.post(taskPostUrl("city_doTaskByTk", { "taskToken": taskToken, "taskId": taskId, "actionType": actionType, "appId": "1GVRQwqc", "safeStr": "" }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
