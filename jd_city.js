...
let uuid, UA;
$.shareCodes = []
let tnum = process.env.CITYNUM || 6;
if ($.isNode()) {
...
let invcode = '';
if (process.env.CITYCODE) {
    if (process.env.CITYCODE.indexOf('&') > -1) {
        invcode = process.env.CITYCODE.split('&');
    } else {
        invcode = [process.env.CITYCODE];
    }
}
!(async () => {
    if (!cookiesArr[0]) {
    ...
    console.log('默认前6个ck开团，变量CITYNUM,运行流程：收集助力码-助力\n');
    if (exchangeFlag) {
        console.log(`自动抽奖`)
    } else {
        console.log(`不自动抽奖，建议活动快结束开启，如需自动抽奖请设置环境变量  JD_CITY_EXCHANGE 为true`);
    }
    if (invcode.length > 0) {
        $.shareCodes = invcode
    } else {
    ...
    console.log('\n开始助力...')
    invcode.length > 0 ? console.log(`当前为指定助力模式!!!`) : console.log(`当前为车头助力模式!!!`);
            ...
            if (res.data?.bizCode === 0) {
                ...
                if (res.data.result.toasts) {
                    if (res.data.result.toasts[0]) {
                        console.log(`助力结果:${res.data.result.toasts[0].msg}`)
                    } else {
                        console.log(`未知错误，跳出`)
                        break
                    }
...
function getInfo(inviteId, flag = false) {
    let body = { "lbsCity": "2", "realLbsCity": "2813", "inviteId": inviteId, "headImg": "", "userName": "", "taskChannel": "4", "location": "", "safeStr": "{\"log\":\"1670634551419~\",\"sceneid\":\"CHFhPageh5\",\"random\":\"51624039\"}" }
                            ...
                            if (data.data && data['data']['bizCode'] === 0) {
                                if (flag) {
                                    ...
                                    for (let pop of data.data.result.popWindows || []) {
                                        if (pop.data.cash && (pop.data.cash !== data.data.result.userActBaseInfo.poolMoney)) {
                                            if (pop && pop.type === "dailycash_second") {
                                                await receiveCash("", "2");
                                                await $.wait(1000);
                                            }
                                        }
                                ...
                                for (let vo of data.data.result && data.data.result.mainInfos || []) {
                                    if (vo && vo.remaingAssistNum === 0 && vo.status === "1") {
                                        ...
                                        if ($.failnum > 2){console.log('领赏金失败过多，换ip或手动领吧！！');break};
...
function tasklist() {
    let body = { "taskChannel": "1", "location": "" };
    return new Promise((resolve) => {
        $.post(taskPostUrl("city_taskDetail", body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    ...
                    if (safeGet(data)) {
                        data = JSON.parse(data);
                        if (data['data']['bizCode'] === 0) {
                            $.tasklist = data.data.result.taskDetailResultVo.taskVos;
                        } else {
                            console.log(data.data)
                        }
                    }
...
function dotask(taskId, taskToken, actionType = 0) {
    return new Promise((resolve) => {
        $.post(taskPostUrl("city_doTaskByTk", { "taskToken": taskToken, "taskId": taskId, "actionType": actionType, "appId": "1GVRQwqc", "safeStr": "{\"log\":\"1670634551419~\",\"sceneid\":\"CHFhPagewx\",\"random\":\"51624039\"}" }), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function taskPostUrl(functionId, body) {
    return {
        url: JD_API_HOST,
        body: `functionId=${functionId}&body=${JSON.stringify(body)}&appid=signed_wh5&osVersion=15.0.1&timestamp=${Date.now()}&&client=ios&clientVersion=11.3.0&openudid=${uuid}`,
        headers: {
            "Host": "api.m.jd.com",
