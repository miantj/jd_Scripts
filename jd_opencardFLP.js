...
/*
10 13 27-31 5 * jd_opencardFLP.js
*/
...
!(async () => {
    ...
    authorCodeList = ['73aa054a1650458297d7a4b06a91f848','360e6813ba2e40adad5e71f9a4e66cb5','c48b62c13ef744269071ea2dc3e7dd13','cf1c15b675d2443598dcac0afc0fe899']
	console.log('\n此活动需要新入会员店铺才能加入队伍，\n若已经入会过，则无法加入。\n');
	console.log('\n瓜分入口:\nhttps://lzkjdz-isv.isvjcloud.com/pool/captain/4471266?activityId=52c0712263f342308da1287a66702009&signUuid=73aa054a1650458297d7a4b06a91f848');
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzkjdz-isv.isvjcloud.com/pool/captain/${$.authorNum}?activityId=${$.activityId}&signUuid=${encodeURIComponent($.authorCode)}&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
			await getUA();
            await FLP();
            await $.wait(3500)
...
async function FLP() {
    ...
    if ($.token) {
        await getMyPing();
        if ($.secretPin) {
            await task('common/accessLogWithAD', `venderId=${$.activityShopId}&code=99&pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&pageUrl=${$.activityUrl}&subType=app&adSource=null`);
			await $.wait(2000)
            ...
            if ($.activityContent) {
                if ($.activityContent.canJoin) {
					...
					if($.errorJoinShop.indexOf('活动太火爆，请稍后再试')>-1){
						...
						allMessage+='【账号'+$.index+'】'+$.UserName+'抱歉，开卡不成功 ，请重新运行脚本\n';
					}
					await $.wait(2000);
                    await task('pool/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&signUuid=${encodeURIComponent($.authorCode)}`)
                    await $.wait(2000)
...
async function joinShop() {
  return new Promise(async resolve => {
    ...
    let body = `{"venderId":"1000003691","shopId":"1000003691","bindByVerifyCodeFlag":1,"registerExtend":{},"writeChildFlag":0,"channel":401}`
...
function getshopactivityId() {
  return new Promise(resolve => {
    const options = {
      url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=getShopOpenCardInfo&body=%7B%22venderId%22%3A%22${$.activityShopId}%22%2C%22channel%22%3A401%7D&client=H5&clientVersion=9.2.0&uuid=88888`,
      headers: {
        'Content-Type': 'text/plain; Charset=UTF-8',
        'Origin': 'https://api.m.jd.com',
        'Host': 'api.m.jd.com',
...
function randomString(_0x4f074c){
	_0x4f074c=_0x4f074c||32;
	let _0x22a3cb='abcdef0123456789',_0x583662=_0x22a3cb.length,_0x25eae0='';
	for(i=0;i<_0x4f074c;i++)_0x25eae0+=_0x22a3cb.charAt(Math.floor(Math.random()*_0x583662));
	return _0x25eae0;
};
